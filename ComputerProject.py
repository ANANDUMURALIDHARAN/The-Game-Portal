...
def snakeGame():
    ...
    medium_font = pygame.font.SysFont('showcard gothic', 30, True)
    large_font = pygame.font.SysFont('chiller', 60, True, True)            #assigning the font and colour to different variable
...
#creating the game initiation window
    def start_game():
        ...
        start_font2 = medium_font.render("Play Game", True, red)  # Creating a surface object from the text
        ...
        start_font2_rect = start_font2.get_rect()  # creating a rectangular surface for each of the
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if x > start_font3_rect.left and x < start_font3_rect.right:
                        if y > start_font3_rect.top and y < start_font3_rect.bottom:
                            portal()
                    if x > start_font2_rect.left and x < start_font2_rect.right:       # creating interactive buttons
    ...
    def Your_score(score):
        ...
        pygame.draw.line(dis, yellow, (540, 0), (540, 40))  #displaying the score
    ...
    def our_snake(snake_block, snake_list):
        ...
        for x in snake_list:
            if n == (len(snake_list) - 1):
                pygame.draw.circle(dis, brown, (x[0], x[1]), 10)     # head of snake

            elif n == 0:
                pygame.draw.circle(dis, red, (x[0], x[1]), 5)        # tail of snake
            else:
                pygame.draw.rect(dis, black, [x[0], x[1], snake_block, snake_block])
            n = n + 1                                                # increasing the length of snake

    def gameLoop():    # defining the game
        game_over = False
        game_close = False

        x1 = dis_width / 2
        y1 = dis_height / 2          # initial position of snake
        ...
        foody = round(random.randrange(70, 400, 10))        # position of food
        ...
        while not game_over:
            ...
            while game_close == True:
                ...
                C = medium_font.render('Press C to continue playing ', True, red)  # display of 'game over' window
                dis.blit(C, [150, 300])
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    game_over = True
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_LEFT:    # movement of snake using keys
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    x, y = event.pos
                    if x > back_rect.left and x < back_rect.right:    # creating back button
                        if y > back_rect.top and y < back_rect.bottom:
                            snakeGame()

            if x1 >= dis_width or x1 < 0 or y1 >= dis_height or y1 <= 40:    # condition to lose game
                game_close = True
            ...
            if x1 == foodx and y1 == foody:  #  condition to gain points
                foodx = round(random.randrange(10, 590, 10))   # assigning a new position to food
                foody = round(random.randrange(70, 400, 10))
                Length_of_snake += 1                           # increasing the length of snake
                pygame.mixer.music.load('snake.mp3')
                pygame.mixer.music.play(0)
            if (Length_of_snake - 1)<5:                       # increasing the speed of the snake after a particular score
                speed=snake_speed
